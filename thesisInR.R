...
results <- ({
  ...
  auxs <- c(carbonTax = carbonTax, windImprovRate = windImprovRate , marginalCostCCNG = marginalCostCCNG)
...
ggplot(results,aes(Time,oPrice))+geom_line()+
  theme_minimal()

ggplot(results,aes(Time,oEmissions))+geom_line()+
  theme_minimal()
